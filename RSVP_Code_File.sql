...
SELECT
	*
FROM
	director_mapping;         # data values of director_mapping table
    
SELECT
	*
FROM
	genre;           # data values of genre table

SELECT
	*
FROM
	names;           # data values of names table

SELECT
	*
FROM
	role_mapping;      # data values of role_mapping table
...
SELECT
	count(*)   ratings_row_count
FROM
	ratings;                           # total number of rows in ratings table

SELECT
	count(*)   director_mapping_row_count
FROM
	director_mapping;         # total number of rows in director_mapping table
    
SELECT
	count(*)   genre_row_count
FROM
	genre;           # total number of rows in genre table

SELECT
	count(*)   names_row_count
FROM
	names;           # total number of rows in names table

SELECT
	count(*)   role_mapping_row_count
FROM
	role_mapping;      # total number of rows in role_mapping table
...
SELECT 
    COUNT(*) AS date_published_nulls       # number of null rows: 0
FROM
    movie
WHERE date_published IS NULL;

SELECT 
    COUNT(*) AS duration_nulls             # number of null rows: 0
FROM
    movie
WHERE duration IS NULL;

SELECT 
    COUNT(*) AS country_nulls        # number of null rows: 20
FROM
    movie
WHERE country IS NULL;

SELECT 
    COUNT(*) AS worlwide_gross_income_nulls       # number of null rows: 3724
FROM
    movie
WHERE worlwide_gross_income IS NULL;

SELECT 
    COUNT(*) AS languages_nulls      # number of null rows: 194
FROM
    movie
WHERE languages IS NULL;

SELECT 
    COUNT(*) AS production_company_nulls       # number of null rows: 528
FROM
    movie
...
/* In Solution 2 above, id in each case statement has been used as a counter to count the number of null values.
   Whenever a value is null for a column, the id increments by 1. */

/* There are 20 nulls in country; 3724 nulls in worlwide_gross_income; 194 nulls in languages; 
   528 nulls in production_company. Notice that we do not need to check for null values in the 
   'id' column as it is a primary key. 
...
/* Most number of movies were released in the year 2017 (3052) and then gradually
 decreased every year(2018 => 2944, 2019 => 2001).*/
...
-- Type your code below:
SELECT 
    MONTH(date_published) month_num, COUNT(*) number_of_movies
FROM
    movie
GROUP BY month_num
...
WHERE
    (country LIKE '%USA%'
        ...
        AND year = '2019';                           # USA and India produced 1059 movies in the year 2019.
...
from genre;                     #  RSVP Movies makes movies on 13 different genres.
...
LIMIT 1;                       #"Drama" genre had the highest number of movies produced overall with count of- 4285
...
( 
SELECT 
    movie_id, COUNT(genre) genre_count
FROM
    genre
GROUP BY movie_id
)
...
FROM
    movie_genre_summary
WHERE
    genre_count = 1;      # 3289 movies belong to only one genre.
...
-- Type your code below:
SELECT 
    genre, AVG(m.duration) avg_duration
FROM
    genre g
        LEFT JOIN
...
FROM
    summary
WHERE
    genre = 'thriller'; /*The rank of the ‘thriller’ genre of movies among all the 
						  genres in terms of number of movies produced is 3. */
...
-- Example: Identify the production companies and their respective rankings based on the number of movies 
--          they have produced.
SELECT 
...
(
    ...
    FROM                                     
		movie AS m                                  #rank() function was used to make a window among production companies and company with higher average rating would get a higher rank.
			LEFT JOIN
		ratings AS r
			ON m.id = r.movie_id
...
-- Q14. How many movies released in each genre in March 2017 in the USA
--     genre had more than 1,000 votes?(Split the question into parts and try to understand it.)
...
-- 54 Movies were released in March 2017 in the USA  having more than 1,000 votes , of which "DRAMA" genre has the most number ,i.e, 16.
-- -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-- Hint: Apply filtering conditions in the WHERE clause using the LIKE operator for the 'title' column an a condition for 'avg_rating'.
...
-- Finally, it selects and rounds the average votes per movie for both German and Italian language movies.
...
-- Answer is Yes , German movies do get more votes than Italian movies.
...
FROM
    names
WHERE
    name IS NULL;         # number of null values in name column is 0
...
FROM
    names
WHERE
    height IS NULL;      # number of null values in height column is 17335
...
FROM
    names
WHERE
    date_of_birth IS NULL;      # number of null values in date_of_birth column is 13431
...
FROM
    names
WHERE
    known_for_movies IS NULL;     # number of null values in known_for_movies column is 15226
...
WITH top_rated_actors AS (
  ...
  FROM 
    ...
    LEFT JOIN movie AS m ON rm.movie_id = m.id 
    INNER JOIN ratings AS r ON m.id = r.movie_id 
  WHERE 
    median_rating >= 8 
  GROUP BY 
...
-- Type your code below:
WITH top_prod
     AS (SELECT m.production_company,   --  Select the production company and the sum of total votes for their movies
                Sum(r.total_votes)                    vote_count,
                Row_number()
                  OVER(
                    ORDER BY Sum(r.total_votes) DESC)
                    prod_comp_rank      -- Assign a rank to each production company based on the total votes in descending order
         FROM   movie m                                
                JOIN ratings r
                  ON r.movie_id = m.id
...
SELECT *
FROM   top_prod
WHERE  prod_comp_rank < 4;       -- Select all columns from the CTE where the rank is less than 4
